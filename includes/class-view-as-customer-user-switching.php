...
class View_As_Customer_User_Switching
{
	/**
	 ...
	 * @var string
	 */
	public static $application = 'WordPress/Upgrade Store';
	...
	public function init_hooks()
	{
		...
		add_filter('user_has_cap', array($this, 'view_as_customer_filter_user_has_cap'), 10, 4);
		add_filter('map_meta_cap', array($this, 'view_as_customer_filter_map_meta_cap'), 10, 4);
		add_filter('user_row_actions', array($this, 'view_as_customer_filter_user_row_actions'), 10, 2);
		add_action('plugins_loaded', array($this, 'view_as_customer_action_plugins_loaded'), 1);
		add_action('init', array($this, 'view_as_customer_action_init'));
		add_action('all_admin_notices', array($this, 'view_as_customer_action_admin_notices'), 1);
		add_action('wp_logout', 'view_as_customer_view_as_customer_clear_olduser_cookie');
		add_action('wp_login', 'view_as_customer_view_as_customer_clear_olduser_cookie');

		// Nice-to-haves:
		add_filter('ms_user_row_actions', array($this, 'view_as_customer_filter_user_row_actions'), 10, 2);
		add_filter('login_message', array($this, 'view_as_customer_filter_login_message'), 1);
		add_filter('removable_query_args', array($this, 'view_as_customer_filter_removable_query_args'));
		add_action('wp_meta', array($this, 'view_as_customer_action_wp_meta'));

		if (isset($view_as_customer_api_settings['fontend_enable']) && $view_as_customer_api_settings['fontend_enable']) {
			add_action('wp_footer', array($this, 'view_as_customer_action_wp_footer'));
			add_action('init', array($this, 'view_as_customer_action_init_body_class'));
		}

		add_action('personal_options', array($this, 'view_as_customer_action_personal_options'));
		add_action('admin_bar_menu', array($this, 'view_as_customer_action_admin_bar_menu'), 11);
		add_action('bbp_template_after_user_details_menu_items', array($this, 'view_as_customer_action_bbpress_button'));
		add_action('woocommerce_login_form_start', array($this, 'view_as_customer_action_woocommerce_login_form_start'), 10, 0);
		add_action('woocommerce_admin_order_data_after_order_details', array($this, 'view_as_customer_action_woocommerce_order_details'), 1);
		add_filter('woocommerce_account_menu_items', array($this, 'view_as_customer_filter_woocommerce_account_menu_items'), 999);
		add_filter('woocommerce_get_endpoint_url', array($this, 'view_as_customer_filter_woocommerce_get_endpoint_url'), 10, 2);
		add_action('view_as_customer_switch_to_user', array($this, 'view_as_customer_forget_woocommerce_session'));
		add_action('view_as_customer_switch_back_user', array($this, 'view_as_customer_forget_woocommerce_session'));


		add_action("admin_bar_menu", array($this, "view_as_customer_admin_bar_menu"), 500);

		add_action("wp_ajax_view_as_customer_show_customer_on_demand", array($this, "view_as_customer_show_customer_on_demand"));
		add_action("wp_ajax_nopriv_view_as_customer_show_customer_on_demand", array($this, "view_as_customer_show_customer_on_demand"));
	}
	/**
	 * Custom
	 * 
	 */
	public function view_as_customer_admin_bar_menu(WP_Admin_Bar $wp_admin_bar)
	{
		...
		$wp_admin_bar->add_menu(array(
			'id'    => 'view-as-customer-switch-as-customer',
			'title' => '<span class="ab-icon"></span><span class="ab-text">' . __('Ultimate View as Customer for WooCommerce', 'view-as-customer') . '</span>',
			'icon' => 'dashicons-before dashicons-admin-post'
			// 'href'  => admin_url('options-general.php?page=perfmatters')
		));
		$wp_admin_bar->add_menu(array(
			'parent' => 'view-as-customer-switch-as-customer',
			'id' => 'view-as-customer-form-in-admin-bar',
			'title' => '<form class="search-user"><div class="input-wrap"><input class="search-user-input" type="text" placeholder="' . __('Start typing to search.', 'view-as-customer') . '" /><div class="search-result"></div></div><button class="search-user-btn" type="button">Search</button> </form>'
		));
		$wp_admin_bar->add_menu(array(
			'parent' => 'view-as-customer-switch-as-customer',
			'id' => 'view-as-customer-current-user-title',
		...
		$wp_admin_bar->add_menu(
			array(
				...
				'id' => 'view-as-customer-switch-user-current',
		...
		$wp_admin_bar->add_menu(array(
			'parent' => 'view-as-customer-switch-as-customer',
			'id' => 'view-as-customer-recent-users-title',
			'title' => '<span class="title-part">Recent Users</span>'
		));
		$view_as_customer_latest_switched_user = get_option('view_as_customer_latest_switched_user') ? array_reverse(get_option('view_as_customer_latest_switched_user')) : [];
		if (sizeof($view_as_customer_latest_switched_user)) {
			$n = 0;
			foreach ($view_as_customer_latest_switched_user as $user_id) {
				$user = get_user_by('id', $user_id);
				$link = self::maybe_switch_url($user);
				$wp_admin_bar->add_menu(
					array(
						'parent' => 'view-as-customer-recent-users-title',
		...
		foreach ($blogusers as $user) {
			...
			$wp_admin_bar->add_menu(
				array(
					'parent' => 'view-as-customer-recent-users-title',
					'title' => $user->display_name . ' (' . ucfirst($user->roles[0]) . ')',
					'id' => 'view-as-customer-switch-user-' . $user->ID,
					'href' => $link
				)
			);
		}*/
		...
		$wp_admin_bar->add_menu(array('parent' => 'view-as-customer-switch-as-customer', 'title' => __('Pending Comments'), 'id' => 'dwb-pending', 'href' => 'edit-comments.php?comment_status=moderated'));
	...
	public function view_as_customer_show_customer_on_demand()
	{
		...
		$html = '';
		$userQuery = sanitize_text_field($_POST['userQuery']);
		global $wpdb;
		$wild = '%';
	 ...
	 * @return void
	 */
	public function view_as_customer_action_plugins_loaded()
	{
		// User Switching's auth_cookie
		if (!defined('VIEW_AS_CUSTOMER_USER_SWITCHING_COOKIE')) {
			define('VIEW_AS_CUSTOMER_USER_SWITCHING_COOKIE', 'wordpress_user_sw_' . COOKIEHASH);
		}

		// User Switching's secure_auth_cookie
		if (!defined('VIEW_AS_CUSTOMER_USER_SWITCHING_SECURE_COOKIE')) {
			define('VIEW_AS_CUSTOMER_USER_SWITCHING_SECURE_COOKIE', 'wordpress_user_sw_secure_' . COOKIEHASH);
		}

		// User Switching's logged_in_cookie
		if (!defined('VIEW_AS_CUSTOMER_USER_SWITCHING_OLDUSER_COOKIE')) {
			define('VIEW_AS_CUSTOMER_USER_SWITCHING_OLDUSER_COOKIE', 'wordpress_user_sw_olduser_' . COOKIEHASH);
		}
	}
	 ...
	 * @return void
	 */
	public function view_as_customer_action_personal_options(WP_User $user)
	{
		$link = self::maybe_switch_url($user);
...
?>
		<tr class="view-as-customer-wrap">
			<th scope="row">
				<?php echo esc_html_x('User Switching', 'User Switching title on user profile screen', 'view-as-customer'); ?>
			</th>
			<td>
				<a id="view_as_customer_switcher" href="<?php echo esc_url($link); ?>">
					<?php esc_html_e('Switch&nbsp;To', 'view-as-customer'); ?>
				</a>
			</td>
		</tr>
		<?php
	...
	public function view_as_customer_action_init_body_class()
	{
		$old_user = self::get_old_user();

		if ($old_user instanceof WP_User) {
			add_filter('body_class', function ($classes) {
				$classes[] = 'view-as-customer-enabled';
				return $classes;
			});
		}
	}
	 ...
	 * @return void
	 */
	public function view_as_customer_action_init()
	{
		$old_user = self::get_old_user();
		...
		switch ($_REQUEST['action']) {
			...
			case 'view_as_customer_switch_to_user':
				$user_id = absint($_REQUEST['user_id'] ?? 0);

				// Check authentication:
				if (!current_user_can('view_as_customer_switch_to_user', $user_id)) {
					wp_die(esc_html__('Could not switch users.', 'view-as-customer'), 403);
				}

				// Check intent:
				check_admin_referer("view_as_customer_switch_to_user_{$user_id}");

				// Switch user:
				$user = view_as_customer_switch_to_user($user_id, self::remember());
				if ($user) {

					// $view_as_customer_latest_switched_user = [];
					$view_as_customer_latest_switched_user = get_option('view_as_customer_latest_switched_user') ? get_option('view_as_customer_latest_switched_user') : [];
					if (in_array($user_id, $view_as_customer_latest_switched_user)) {
						array_splice($view_as_customer_latest_switched_user, array_search($user_id, $view_as_customer_latest_switched_user), 1);
					}
					$view_as_customer_latest_switched_user[] = $user_id;
					update_option('view_as_customer_latest_switched_user', $view_as_customer_latest_switched_user);
					...
					wp_die(esc_html__('Could not switch users.', 'view-as-customer'), 404);
				}
				break;
			...
			case 'switch_to_olduser':
				...
				$old_user = self::get_old_user();
				if (!$old_user) {
					wp_die(esc_html__('Could not switch users.', 'view-as-customer'), 400);
				}
				...
				// Switch user:
				if (view_as_customer_switch_to_user($old_user->ID, self::remember(), false)) {
			...
			case 'switch_off':
				...
				if (!$current_user || !current_user_can('switch_off')) {
					...
					wp_die(esc_html__('Could not switch off.', 'view-as-customer'), 403);
				}
				...
				// Switch off:
				if (view_as_customer_switch_off_user()) {
	...
	{
		$nonce = wp_create_nonce('view_as_customer_security_nonce');
		if (!wp_verify_nonce(sanitize_text_field(wp_unslash($nonce)), 'view_as_customer_security_nonce')) {
			wp_send_json_error('Nonce validation failed.');
			die();
		}
		 ...
		 * @param string|null  $redirect_type The redirect type, see the `View_As_Customer_User_Switching::REDIRECT_*` constants.
		 ...
		 */
		return apply_filters('view_as_customer_redirect_to', $redirect_to, $redirect_type, $new_user, $old_user);
	}
	 ...
	 * @return void
	 */
	public function view_as_customer_action_admin_notices()
	{
		...
		?>
			<div id="view_as_customer" class="updated notice notice-success is-dismissible">
				<?php
				if ($lang_attr) {
					printf(
						'<p lang="%s">',
				 ...
				 */
				$message = apply_filters('view_as_customer_switched_message', $message, $user, $old_user, $switch_back_url, $just_switched);
	...
	{
		$cookie = view_as_customer_get_olduser_cookie();
		if (!empty($cookie)) {
	...
	{
		$cookie = view_as_customer_get_auth_cookie();
		if (!empty($cookie)) {
	 ...
	 * @return void
	 */
	public function view_as_customer_action_admin_bar_menu(WP_Admin_Bar $wp_admin_bar)
	{
		...
		if (current_user_can('switch_off')) {
			...
			// 	'id' => 'switch-off',
			// 	// Translators: "switch off" means to temporarily log out
			// 	'title' => esc_html__('Switch Off', 'view-as-customer'),
			// 	'href' => $url,
			// ));
		}
		...
		if (!is_admin() && is_author() && (get_queried_object() instanceof WP_User)) {
			...
			} elseif (current_user_can('view_as_customer_switch_to_user', get_queried_object_id())) {
				$wp_admin_bar->add_node(array(
					'parent' => 'edit',
					'id' => 'author-switch-to',
					'title' => esc_html__('Switch&nbsp;To', 'view-as-customer'),
					'href' => add_query_arg(array(
						'redirect_to' => rawurlencode(self::current_url()),
	 ...
	 * @return void
	 */
	public function view_as_customer_action_wp_meta()
	{
		$old_user = self::get_old_user();
		...
		if ($old_user instanceof WP_User) {
			...
			printf(
				'<li id="view_as_customer_switch_on_list"><a href="%s">%s</a></li>',
				esc_url($url),
				esc_html(self::switch_back_message($old_user))
			);
		}
	 ...
	 * @return void
	 */
	public function view_as_customer_action_wp_footer()
	{
		 ...
		 */
		if (!apply_filters('view_as_customer_in_footer', true)) {
			return;
		}
		...
		?>
			<div id="view_as_customer_switch_on"><span class="current-user-data"><?php echo esc_html__('You\'re Viewing as', 'view-as-customer') . ' ' . esc_html(wp_get_current_user()->display_name) ?></span><span class="switch-to-old"><a href="<?php echo esc_url($url) ?>"><?php echo esc_html(self::switch_back_message($old_user)) ?></a></span></div>
<?php
		}
	}
	 ...
	 */
	public function view_as_customer_filter_login_message($message)
	{
		...
		if ($old_user instanceof WP_User) {
			...
			$message .= '<p class="message" id="view_as_customer_switch_on">';
	 ...
	 */
	public function view_as_customer_filter_user_row_actions(array $actions, WP_User $user)
	{
		$link = self::maybe_switch_url($user);
		...
		$actions['view_as_customer_switch_to_user'] = sprintf(
			'<a href="%s">%s</a>',
			esc_url($link),
			esc_html__('Switch&nbsp;To', 'view-as-customer')
		);
	 ...
	 * @return void
	 */
	public function view_as_customer_action_bbpress_button()
	{
		...
		echo '<ul id="view_as_customer_switch_to">';
		printf(
			'<li><a href="%s">%s</a></li>',
			esc_url($link),
			esc_html__('Switch&nbsp;To', 'view-as-customer')
	 ...
	 */
	public function view_as_customer_filter_removable_query_args(array $args)
	{
	...
	public static function maybe_switch_url(WP_User $user)
	{
		...
		} elseif (current_user_can('view_as_customer_switch_to_user', $user->ID)) {
	...
	public static function switch_to_url(WP_User $user)
	{
		return wp_nonce_url(add_query_arg(array(
			'action' => 'view_as_customer_switch_to_user',
			'user_id' => $user->ID,
			'nr' => 1,
		), wp_login_url()), "view_as_customer_switch_to_user_{$user->ID}");
	}
	...
	public static function switched_to_message(WP_User $user)
	{
		$message = sprintf(
			...
			__('Switched to %1$s (%2$s).', 'view-as-customer'),
			$user->display_name,
			$user->user_login
		);
	...
	public static function switch_back_message(WP_User $user)
	{
		$message = sprintf(
			/* Translators: 1: user display name; 2: username; */
			__('Switch back to %1$s (%2$s)', 'view-as-customer'),
			$user->display_name,
			$user->user_login
		);
	...
	public static function switched_back_message(WP_User $user)
	{
		$message = sprintf(
			/* Translators: 1: user display name; 2: username; */
			__('Switched back to %1$s (%2$s).', 'view-as-customer'),
			$user->display_name,
			$user->user_login
		);
	 ...
	 * @return void
	 */
	public function view_as_customer_action_woocommerce_login_form_start()
	{
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo wp_kses_post($this->view_as_customer_filter_login_message(''));
	}
	 ...
	 * @return void
	 */
	public function view_as_customer_action_woocommerce_order_details(WC_Order $order)
	{
		$user = $order->get_user();

		if (!$user || !current_user_can('view_as_customer_switch_to_user', $user->ID)) {
			return;
		}
	 ...
	 */
	public function view_as_customer_filter_woocommerce_account_menu_items(array $items)
	{
		$old_user = self::get_old_user();
		...
		$items['view-as-customer-switch-back'] = self::switch_back_message($old_user);
	 ...
	 */
	public function view_as_customer_filter_woocommerce_get_endpoint_url($url, $endpoint)
	{
		if ('view-as-customer-switch-back' !== $endpoint) {
			return $url;
		}
	 ...
	 * @return void
	 */
	public function view_as_customer_forget_woocommerce_session()
	{
	 ...
	 * This is used to:
	 *
	 *  - Grant the 'view_as_customer_switch_to_user' capability to the user if they have the ability to edit the user they're trying to
	 *    switch to (and that user is not themselves).
	 *  - Grant the 'switch_off' capability to the user if they can edit other users.
	 *
	 * Important: This does not get called for Super Admins. See view_as_customer_filter_map_meta_cap() below.
	 *
	 ...
	 */
	public function view_as_customer_filter_user_has_cap(array $user_caps, array $required_caps, array $args, WP_User $user)
	{
		if ('view_as_customer_switch_to_user' === $args[0]) {
			if (empty($args[2])) {
				$user_caps['view_as_customer_switch_to_user'] = false;
				return $user_caps;
			}
			if (array_key_exists('switch_users', $user_caps)) {
				$user_caps['view_as_customer_switch_to_user'] = $user_caps['switch_users'];
				return $user_caps;
			}

			$user_caps['view_as_customer_switch_to_user'] = (user_can($user->ID, 'edit_user', $args[2]) && ($args[2] !== $user->ID));
	 ...
	 */
	public function view_as_customer_filter_map_meta_cap(array $required_caps, $cap, $user_id, array $args)
	{
		if ('view_as_customer_switch_to_user' === $cap) {
	 ...
	 *
	 * @return View_As_Customer_User_Switching User Switching instance.
	 */
	public static function get_instance()
	{
		...
		if (!isset($instance)) {
			$instance = new View_As_Customer_User_Switching();
		}
...
if (!function_exists('view_as_customer_view_as_customer_set_olduser_cookie')) {
	/**
	 ...
	 * @return void
	 */
	function view_as_customer_view_as_customer_set_olduser_cookie($old_user_id, $pop = false, $token = '')
	{
		$secure_auth_cookie = View_As_Customer_User_Switching::secure_auth_cookie();
		$secure_olduser_cookie = View_As_Customer_User_Switching::secure_olduser_cookie();
		$expiration = time() + 172800; // 48 hours
		$auth_cookie = view_as_customer_get_auth_cookie();
		$olduser_cookie = wp_generate_auth_cookie($old_user_id, $expiration, 'logged_in', $token);

		if ($secure_auth_cookie) {
			$auth_cookie_name = VIEW_AS_CUSTOMER_USER_SWITCHING_SECURE_COOKIE;
			$scheme = 'secure_auth';
		} else {
			$auth_cookie_name = VIEW_AS_CUSTOMER_USER_SWITCHING_COOKIE;
			$scheme = 'auth';
		}
		 ...
		 */
		do_action('view_as_customer_set_cookie', $auth_cookie, $expiration, $old_user_id, $scheme, $token);
		 ...
		 */
		do_action('view_as_customer_set_olduser_cookie', $olduser_cookie, $expiration, $old_user_id, $scheme, $token);
		 ...
		 */
		if (!apply_filters('view_as_customer_send_auth_cookies', true)) {
			return;
		}

		setcookie($auth_cookie_name, $auth_cookie, $expiration, SITECOOKIEPATH, COOKIE_DOMAIN, $secure_auth_cookie, true);
		setcookie(VIEW_AS_CUSTOMER_USER_SWITCHING_OLDUSER_COOKIE, $olduser_cookie, $expiration, COOKIEPATH, COOKIE_DOMAIN, $secure_olduser_cookie, true);
	}
}

if (!function_exists('view_as_customer_view_as_customer_clear_olduser_cookie')) {
	/**
	 ...
	 * @return void
	 */
	function view_as_customer_view_as_customer_clear_olduser_cookie($clear_all = true)
	{
		$auth_cookie = view_as_customer_get_auth_cookie();
		...
		if ($clear_all || empty($auth_cookie)) {
			 ...
			 * @since 1.4.0
			 */
			do_action('view_as_customer_clear_olduser_cookie');
			...
			setcookie(VIEW_AS_CUSTOMER_USER_SWITCHING_COOKIE,         ' ', $expire, SITECOOKIEPATH, COOKIE_DOMAIN);
			setcookie(VIEW_AS_CUSTOMER_USER_SWITCHING_SECURE_COOKIE,  ' ', $expire, SITECOOKIEPATH, COOKIE_DOMAIN);
			setcookie(VIEW_AS_CUSTOMER_USER_SWITCHING_OLDUSER_COOKIE, ' ', $expire, COOKIEPATH, COOKIE_DOMAIN);
		} else {
			if (View_As_Customer_User_Switching::secure_auth_cookie()) {
				$scheme = 'secure_auth';
			} else {
				$scheme = 'auth';
			}
			...
			if ($old_user_id) {
				...
				if (false !== $parts) {
					view_as_customer_view_as_customer_set_olduser_cookie($old_user_id, true, $parts['token']);
				}
			}
		}
	}
...
if (!function_exists('view_as_customer_get_olduser_cookie')) {
	/**
	 ...
	 */
	function view_as_customer_get_olduser_cookie()
	{
		if (isset($_COOKIE[VIEW_AS_CUSTOMER_USER_SWITCHING_OLDUSER_COOKIE])) {
			return sanitize_text_field(wp_unslash($_COOKIE[VIEW_AS_CUSTOMER_USER_SWITCHING_OLDUSER_COOKIE]));
		} else {
			return false;
		}
	}
...
if (!function_exists('view_as_customer_get_auth_cookie')) {
	/**
	 ...
	 */
	function view_as_customer_get_auth_cookie()
	{
		if (View_As_Customer_User_Switching::secure_auth_cookie()) {
			$auth_cookie_name = VIEW_AS_CUSTOMER_USER_SWITCHING_SECURE_COOKIE;
		} else {
...
if (!function_exists('view_as_customer_switch_to_user')) {
	/**
	 ...
	 */
	function view_as_customer_switch_to_user($user_id, $remember = false, $set_old_user = true)
	{
		...
		$auth_cookies = view_as_customer_get_auth_cookie();
		...
		if ($set_old_user && $old_user_id) {
			...
			$new_token = '';
			view_as_customer_view_as_customer_set_olduser_cookie($old_user_id, false, $old_token);
		} else {
			// Switching back, either after being switched off or after being switched to another user
			$new_token = $cookie_parts['token'] ?? '';
			view_as_customer_view_as_customer_clear_olduser_cookie(false);
		}
		...
		if ($set_old_user && $old_user_id) {
			 ...
			 */
			do_action('view_as_customer_switch_to_user', $user_id, $old_user_id, $new_token, $old_token);
		} else {
			/**
			 ...
			 * @param string    $old_token   The token of the session of the user being switched from.
			 */
			do_action('view_as_customer_switch_back_user', $user_id, $old_user_id, $new_token, $old_token);
		}
...
if (!function_exists('view_as_customer_switch_off_user')) {
	/**
	 ...
	 */
	function view_as_customer_switch_off_user()
	{
		 ...
		 */
		do_action('view_as_customer_switch_off_user', $old_user_id, $old_token);

		return true;
	}
}

if (!function_exists('view_as_customer_current_user_switched')) {
	/**
	 ...
	 */
	function view_as_customer_current_user_switched()
	{
		if (!is_user_logged_in()) {
			return false;
		}

		return View_As_Customer_User_Switching::get_old_user();
	}
}

$GLOBALS['view_as_customer'] = View_As_Customer_User_Switching::get_instance();
$GLOBALS['view_as_customer']->init_hooks();
