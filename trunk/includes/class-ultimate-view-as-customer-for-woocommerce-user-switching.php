...
class Ultimate_View_As_Customer_For_Woocommerce_User_Switching
{
	...
	public function init_hooks()
	{
		...
		if (isset($ultimate_view_as_customer_for_woocommerce_api_settings['fontend_enable']) && $ultimate_view_as_customer_for_woocommerce_api_settings['fontend_enable']) {
	...
	public function ultimate_view_as_customer_for_woocommerce_show_customer_on_demand()
	{
		...
		$old_user_id = isset($_POST['old_user_id']) ? sanitize_text_field($_POST['old_user_id']) : '';
	...
	public function ultimate_view_as_customer_for_woocommerce_action_init()
	{
		...
		switch ($_REQUEST['action']) {
			...
			case 'customer_to_customer':
				$user = get_user_by('id', $_REQUEST['sw_user']);
				$link = self::maybe_switch_url($user);
...
<?php
	...
	public static function switched_back_message(WP_User $user)
	{
		$message = sprintf(
			/* Translators: 1: user display name; 2: username; */
			__('Switched back to %1$s (%2$s).', 'ultimate-view-as-customer-for-woocommerce'),
			$user->display_name,
			$user->user_login
		);
